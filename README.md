...
- **Backend**: Python + Flask, lưu dữ liệu vào file SQLite (`tracker.db`) ngay trong thư mục này.
- **Frontend**: HTML/CSS/JS thuần (Chart.js qua CDN) — không cần cài Node/npm.
- Mọi người mở trình duyệt trỏ vào máy bạn, dữ liệu đổ thẳng vào 1 database dùng chung, dashboard tự refresh mỗi 15 giây.
...
- Trên chính máy bạn: mở trình duyệt vào `http://localhost:5000`
- Để đồng nghiệp trong cùng mạng Wifi/LAN văn phòng truy cập được: tìm địa chỉ IP máy bạn:
  ...
  - Gửi cho team địa chỉ: `http://<IP đó>:5000` (ví dụ `http://192.168.1.23:5000`)
...
Hệ thống **không có cơ chế đăng nhập/xác thực**. Bất kỳ ai truy cập được vào địa chỉ web (kể cả qua LAN) đều có thể nhập, sửa, hoặc xoá dữ liệu — kể cả các danh mục Test Suite/Model/Owner ở tab Cài đặt. Phù hợp cho team nội bộ tin cậy lẫn nhau trong mạng văn phòng; không phù hợp nếu cần kiểm soát ai được làm gì một cách chặt chẽ.

## Sao lưu / di chuyển dữ liệu

Toàn bộ dữ liệu nằm trong 1 file duy nhất: `tracker.db`. Muốn backup, chỉ cần copy file này.
